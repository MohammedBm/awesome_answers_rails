# This file helps us define all authrozeing rule for our application we can use these rules in our controller and views throughout our application
...
class Ability
  ...
  def initialize(user)
    ...
    #In this rule we're saying: the user can `manage` meaning do any action on the question object if `ques.user == user` which means if the owner of the question is the currently signed in user
    ...
    can :like, Question do |ques|
      ques.user != user
    end

    cannot :like, Question do |ques|
      ques.user == user
    end
